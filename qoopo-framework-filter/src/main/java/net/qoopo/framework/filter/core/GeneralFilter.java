...
public class GeneralFilter {
        ...
        public static Condition condicionActivo() {
                return Condition.getOpen().concat(
                                Condition.builder().field(GeneralFilter.ARCHIVADO).function(Function.IGUAL)
                                                ...
                                                .or(Condition.builder().field(GeneralFilter.ARCHIVADO)
                                                                .function(Function.ES_NULO).build())
                                                .close())
                                .setName("Activo");
        }
        ...
        public static ConditionCollection dateField(Field field) {
                return ConditionCollection.of(
                                field.getNombre(),
                                List.of(GeneralFilter.fechaHoy(field),
                                                ConditionCollection.of("Mes",
                                                                List.of(GeneralFilter.fechaEsteMes(field),
                                                                                GeneralFilter.fechaMesAnterior(field),
                                                                                monthsOfYear(String.valueOf(LocalDate
                                                                                                .now().getYear()),
                                                                                                field, LocalDate.now()),
                                                                                monthsOfYear(String.valueOf(LocalDate
                                                                                                .now().plusYears(-1)
                                                ...
                                                ConditionCollection.of("Trimestre",
                                                                List.of(quarterOfYear(String.valueOf(LocalDate
                                                                                .now().getYear()),
                                                                                field, LocalDate.now()),
                                                                                quarterOfYear(String.valueOf(LocalDate
                                                                                                .now().plusYears(-1)
                                                                                                .getYear()), field,
                                                                                                LocalDate.now().plusYears(
                                                                                                                -1)),
                                                ...
                                                ConditionCollection.of("Año",
                                                                List.of(GeneralFilter.year(field,
                                                                                LocalDate.now()),
                                                                                GeneralFilter.year(field, LocalDate
                                                                                                .now().plusYears(-1)),
                                                                                GeneralFilter.year(field, LocalDate
                                                                                                .now().plusYears(-2)),
                                                                                GeneralFilter.year(field, LocalDate
        ...
        public static ConditionCollection monthsOfYear(String name, Field field, LocalDate date) {
                ...
                for (int i = 1; i <= 12; i++) {
                        // LocalDate endDate =
                        // FechasUtil.convertToLocalDateViaInstant(FechasUtil.fechaFinMes(FechasUtil.convertLocalDate(startDate)));
                        LocalDate endDate = FechasUtil.fechaFinMes(startDate);
        ...
        public static ConditionCollection quarterOfYear(String name, Field field, LocalDate date) {
                List<Condition> lstMonts = new ArrayList<>();
                LocalDate startDate = FechasUtil.fechaInicioAnio(date).toLocalDate();
                for (int i = 1; i <= 4; i++) {

                        LocalDate endDate = FechasUtil.fechaFinMes(startDate.plusMonths(2));
                        lstMonts.add(dateRange(field, startDate, endDate)
                                        .setName(startDate.getMonth().getDisplayName(TextStyle.FULL, Locale.US)
                                                        + "-" +
        ...
        public static Condition fechaHoy(Field field) {
                return dateRange(field, FechasUtil.fechaInicio(LocalDateTime.now()),
                                FechasUtil.fechaFin(LocalDateTime.now()))
                                .setName("Hoy");
        ...
        public static Condition month(Field field, LocalDate date) {
                return dateRange(field, FechasUtil.fechaInicioMes(LocalDateTime.now()),
                                ...
                                .setName(
                                                String.valueOf(date.getMonth().getDisplayName(TextStyle.FULL_STANDALONE,
                                                                Locale.getDefault())));
        }
